...
LOGFOLDER="${VOLUME}/Backup/${VERSION}/CarPlayRGI-new"
RESULT=0
...
echo "===== Collecting CarPlay RGI New runtime logs ====="
...
copy_runtime_log "/tmp/carplay_wrapper.log" "carplay_wrapper.log"
...
if [ "${RESULT}" -eq 0 ]; then
  echo "CarPlay RGI New runtime logs collected successfully."
else
  echo "CarPlay RGI New runtime log collection completed with errors."
fi

echo "===== CarPlay RGI New runtime log collection finished ====="
exit "${RESULT}"
